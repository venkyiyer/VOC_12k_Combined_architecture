...
def main():
    ...
    with tf.Session(config=config) as sess:
        ...
        for e in range(start_epoch, args.epochs):
            ...
            #if not os.path.isdir("%s/%04d" % ("Training_imgs_fcn32", e)):
            #    os.makedirs("%s/%04d" % ("Training_imgs_fcn32", e))

            #counter = 0

            #target = open("%s/%04d/val_scores.csv" % ("Training_imgs_fcn32", e), 'w')
            ...
            for x, y, gt_boxes, img_seg_gt, imgseg_gt_to_compare in tqdm(generator, total=n_train_batches,
                ...
                cv2.imwrite('rgb_image.jpg', np.squeeze(x))
                cv2.imwrite('image_to_compare.png', np.squeeze(imgseg_gt_to_compare))
                rev_img_seg_gt = reverse_one_hot(img_seg_gt)
                color_img_seg_gt = colour_code_segmentation(rev_img_seg_gt)
                cv2.imwrite('label.png',color_img_seg_gt)
                exit()
                ...
                output_seg, result, loss_batch, _ = sess.run([net.logits_seg, net.result, net.losses, net.optimizer],feed_dict=feed)
                ...
                #cv2.imwrite("%s/%04d/%s_gt.png" % ("Training_imgs_fcn32", e, filename), training_gt)
                #cv2.imwrite("%s/%04d/%s_pred.png" % ("Training_imgs_fcn32", e, filename), output_seg_output)
            ...
            if not os.path.isdir("%s/%04d" % ("Validation_imgs_fcn32", e)):
                os.makedirs("%s/%04d" % ("Validation_imgs_fcn32", e))

            counter_val =0

            target = open("%s/%04d/val_scores.csv" % ("Validation_imgs_fcn32", e), 'w')
            target.write("val_name, avg_accuracy, precision, recall, f1 score, mean iou, %s\n" % str(counter_val))
            ...
            for x, y, gt_boxes, img_seg_gt, imgseg_gt_to_compare in tqdm(generator, total=n_valid_batches,
                ...
                cv2.imwrite("%s/%04d/%s_gt.png" % ("Validation_imgs_fcn32", e, filename), original_gt)
                cv2.imwrite("%s/%04d/%s_pred.png" % ("Validation_imgs_fcn32", e, filename), output_seg_output)
